...
function assumeLambdaRole () {
    return new Promise( ( resolve, reject ) => {
        ...
        const roleArn = getLambdaConfigFile().deployedAssets[ process.env.ENV ].roleArn;
...
function copyAllFilesToDistDir () {
    return new Promise( ( resolve, reject ) => {
        ...
        webpack({
            ...
            stats.compilation.fileDependencies.forEach( path => {
            ...
            // delete webpack output file, we don't technically need it as we don't want a single concated file
            // fs.unlinkSync( `${lambdaDeployStagingDir}/index.js` );
...
function copyFile ( filePath ) {
    ncp( `./${filePath}`, `./${lambdaDeployStagingDir}/${filePath}`, ( err ) => {
        if ( err ) {
            console.log( err );
        }
    } );
    if ( isNodeModule( filePath ) ) {
        const modulePath = filePath.split( "/" ).slice( 0, 2 );
        modulePath.push( "package.json" );
        const packageJsonPath = modulePath.join( "/" );
        ncp( `./${packageJsonPath}`, `./${lambdaDeployStagingDir}/${packageJsonPath}`, ( err ) => {
            if ( err ) {
                console.log( err );
            }
        } );
...
function zipFile ( package ) {
    ...
    return new Promise( ( resolve, reject ) => {
        ...
        cmd.get( `cd ${lambdaDeployStagingDir} && zip -9 -r ${process.env.COLLY__LAMBDA_NAME}.zip ${process.env.COLLY__LAMBDA_NAME}`, ( err, stdout ) => {
