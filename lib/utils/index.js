...
function assumeLambdaRole () {
    return new Promise( ( resolve, reject ) => {
        ...
        const roleArn = getValueFromLambdaConfig( `deployedAssets.${process.env.LAMBDA__ENV}.${getLambdaName()}RoleArn` );
...
function anyEnvButLive () {
    return ( process.env.LAMBDA__ENV !== "live" ) ? process.env.LAMBDA__ENV.toUpperCase() : "";
}
...
function getProjectConfigFilePath () {
    return `${process.env.COLLY__PROJECT_DIR}/${ chooseProjectFile( listEnvFiles(), process.env.LAMBDA__ENV ) }`;
}
...
// removes "./" from "./foo/bar"
function removeRelativeStartOfPath ( path ) {
    return path.replace(/^.?\//, "");
}

function addForwardSlashToEndOfDirPath ( path ) {
    let append = "";
    if ( path.substr( - 1 ) !== "/" ) {
        append = "/";
    }
...
function getLambdaFilePath ( altStartOfPath ) {
    ...
    const startOfPath = addForwardSlashToEndOfDirPath(
        addForwardSlashToEndOfDirPath( process.env.COLLY__PROJECT_DIR ) + ( removeRelativeStartOfPath( altStartOfPath || "" ) )
    );
...
function prependToRelativeLambdaPath ( overridePath ) {

    const relativePathToProjectDir = process.env.COLLY__PROJECT_DIR.split( process.cwd() )[ 1 ];

    const startOfPath = addForwardSlashToEndOfDirPath(
        "." + addForwardSlashToEndOfDirPath( relativePathToProjectDir ) + ( removeRelativeStartOfPath( overridePath || "" ) )
    );
...
function prepLambdaDeployStagingDir () {
    return new Promise( ( resolve ) => {
        ...
        del.sync( [ addForwardSlashToEndOfDirPath( process.env.COLLY__PROJECT_DIR ) + lambdaDeployStagingDir ] );
        resolve();
...
function copyAllFilesToDistDir () {
    return new Promise( ( resolve, reject ) => {
        ...
        webpack({
            ...
            "output": {
                "filename": prependToRelativeLambdaPath( `./${lambdaDeployStagingDir}` ),
            ...
            Promise.all( stats.compilation.fileDependencies.map( path => {
                ...
                return new Promise( ( resolve, reject ) => {
                    ...
                    makeDir( addStagingDirIntoRelativeFilePath( relativePathToDir ) )
                        .then( () => {
                            copyFile( relativePath ).then( resolve ).catch( reject );
                        })
                        .catch( () => {
                            copyFile( relativePath ).then( resolve ).catch( reject );
                        });
                ...
                resolve( `${process.env.COLLY__PROJECT_DIR}/${lambdaDeployStagingDir}/${process.env.COLLY__LAMBDA_NAME}` );
...
function addStagingDirIntoRelativeFilePath ( path ) {
    const pieces = ( `${process.cwd()}/${path}` ).split( process.env.COLLY__PROJECT_DIR );
    return `.${process.env.COLLY__PROJECT_DIR.split( process.cwd() )[ 1 ]}/${lambdaDeployStagingDir}${pieces[ 1 ]}`;
}
...
function copyFile ( filePath ) {
    return new Promise( ( resolve, reject ) => {
        ...
        let filesToCopy = [{
            "from": `./${filePath}`,
            "to": addStagingDirIntoRelativeFilePath( filePath )
        }];
        ...
        if ( isNodeModule( filePath ) ) {
            ...
            filesToCopy.push({
                ...
                "to": addStagingDirIntoRelativeFilePath( packageJsonPath )
            });
        }
        ...
        if ( isNodeModuleUglifyJS( filePath ) ) {
            ...
            filesToCopy.push({
                ...
                "to": addStagingDirIntoRelativeFilePath( uglifyJSLibPath )
            });
            ...
            if ( fs.existsSync( `./${exportsJsPath}` ) ) {
                filesToCopy.push({
                    "from": `./${exportsJsPath}`,
                    "to": addStagingDirIntoRelativeFilePath( exportsJsPath )
                });
            }
        ...
        if ( isNodeModuleMime( filePath ) ) {
            ...
            if ( fs.existsSync( `./${mimeTypesPath}` ) ) {
                filesToCopy.push({
                    "from": `./${mimeTypesPath}`,
                    "to": addStagingDirIntoRelativeFilePath( mimeTypesPath )
                });
            }
        ...
        // Another hack similiar to the one above
        if ( isNodeModuleGoogleApis( filePath ) ) {

            const googleApisLibPath = pathToOtherModule( filePath, "apis" );
            filesToCopy.push({
                "from": `./${googleApisLibPath}`,
                "to": addStagingDirIntoRelativeFilePath( googleApisLibPath )
            });
...
function isNodeModuleGoogleApis ( path ) {
    const pathDirectories = path.split( "/" ).reverse();
    return pathDirectories[ 0 ] === "googleapis.js";
}
...
function setOptions ( cliOptions ) {
    ...
    if ( cliOptions.env ) {
        process.env.LAMBDA__ENV = cliOptions.env;
    } else {
    ...
    process.env.COLLY__DRY_RUN = ( options.dry_run ) ? true : false;
...
function zipFile ( package ) {
    ...
    return new Promise( ( resolve, reject ) => {
        ...
        cmd.get( `cd ${process.env.COLLY__PROJECT_DIR}/${lambdaDeployStagingDir} && zip -9 -r ${process.env.COLLY__LAMBDA_NAME}.zip .`, ( err, stdout ) => {
...
function copyAssetToLambdaDeployStagingDir ( asset ) {

    return new Promise ( ( resolve, reject ) => {

        pathType.file( `./${asset}` ).then( ( isFile ) => {
            isFile && cpFile( `./${asset}`, `${process.env.COLLY__PROJECT_DIR}/${lambdaDeployStagingDir}/${asset}` ).then( resolve );
        });
        pathType.dir( `./${asset}` ).then( ( isDir ) => {
            isDir && ncp( `./${asset}`, `${process.env.COLLY__PROJECT_DIR}/${lambdaDeployStagingDir}/${asset}`, resolve );
        });
...
function copyAdditionalFiles ( packagename ) {
    return new Promise( ( resolve, reject ) => {
        ...
        if ( additionalDeploymentAssets.length > 0 ) {
            const promises = additionalDeploymentAssets.map( copyAssetToLambdaDeployStagingDir );
...
function getLambdaRoleArnKey() {
    ...
    return getValueFromLambdaConfig( `deployedAssets.${process.env.LAMBDA__ENV}.${getLambdaName()}RoleArn` ) || getValueFromLambdaConfig( `deployedAssets.${process.env.LAMBDA__ENV}.roleArn` );
}
...
module.exports = {
    ...
    "prependToRelativeLambdaPath": prependToRelativeLambdaPath,
    "pathToOtherModule": pathToOtherModule,
    "addStagingDirIntoRelativeFilePath": addStagingDirIntoRelativeFilePath,
