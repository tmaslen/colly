...
function getListOfAdditionalDeploymentAssets () {
    let list = [];
    const projectConfig = getProjectConfig();
    const additionalFilesForLambda = getValueFromLambdaConfig( "additionalDeploymentAssets" );
    if ( "additionalDeploymentAssets" in projectConfig ) {
        list = list.concat( projectConfig.additionalDeploymentAssets );
    }
    if ( additionalFilesForLambda ) {
        list = list.concat( additionalFilesForLambda );
    }
    return list;
}
