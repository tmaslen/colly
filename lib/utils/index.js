    ...
    return new Promise( ( resolve, reject ) => {
        ...
        if ( isNodeModuleGoogleApis( filePath ) ) {
            ...
            filesToCopy.push({
                ...
                "to": `./${lambdaDeployStagingDir}/${googleApisLibPath}`
            });
...
function setOptions ( cliOptions ) {
    ...
    process.env.COLLY__DEPLOY_DRY_RUN = ( options.deploy_dry_run ) ? true : false;
...
function copyAssetToLambdaDeployStagingDir ( asset ) {
    ...
    return new Promise ( ( resolve, reject ) => {
        ...
        pathType.file( `./${asset}` ).then( ( isFile ) => {
            isFile && cpFile( `./${asset}`, `./${lambdaDeployStagingDir}/${asset}` ).then( resolve );
        });
        pathType.dir( `./${asset}` ).then( ( isDir ) => {
            isDir && ncp( `./${asset}`, `./${lambdaDeployStagingDir}/${asset}`, resolve );
        });
